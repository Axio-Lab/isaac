      ...
      <div className="w-full max-w-sm border border-border rounded-xl p-5 bg-card">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          <div>
            ...
            <input type="text" placeholder="Your name" {...register("name")} className="mt-1 w-full h-9 px-3 border border-border rounded-lg text-xs bg-background text-foreground focus:outline-none focus:ring-1 focus:ring-ring" />
            {errors.name && <p className="text-[10px] text-destructive mt-1">{errors.name.message}</p>}
          </div>
          <div>
            <label className="text-[11px] font-medium text-muted-foreground">Email</label>
            <input type="email" placeholder="user@example.com" {...register("email")} className="mt-1 w-full h-9 px-3 border border-border rounded-lg text-xs bg-background text-foreground focus:outline-none focus:ring-1 focus:ring-ring" />
          ...
          <div>
            ...
            <input type="password" placeholder="********" {...register("password")} className="mt-1 w-full h-9 px-3 border border-border rounded-lg text-xs bg-background text-foreground focus:outline-none focus:ring-1 focus:ring-ring" />
          ...
          <div>
            ...
            <input type="password" placeholder="********" {...register("confirmPassword")} className="mt-1 w-full h-9 px-3 border border-border rounded-lg text-xs bg-background text-foreground focus:outline-none focus:ring-1 focus:ring-ring" />
            {errors.confirmPassword && <p className="text-[10px] text-destructive mt-1">{errors.confirmPassword.message}</p>}
          </div>

          <GlassButton type="submit" disabled={isSubmitting || socialLoading} size="sm" className="w-full glass-filled" contentClassName="flex items-center justify-center w-full text-xs font-semibold py-2.5">
            {isSubmitting ? "Creating account..." : "Create account"}
          </GlassButton>

          <p className="text-center text-[11px] text-muted-foreground">
            Already have an account?{" "}
            <Link href="/login" className="text-primary hover:underline underline-offset-4 font-medium">Sign in</Link>
          </p>
        </form>
      </div>
    </div>
